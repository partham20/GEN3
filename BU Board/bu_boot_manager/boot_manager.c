/*
 * boot_manager.c  —  BU-Board variant
 *
 * Boot Manager for the BU board (F28P550SJ9). Identical logic to the
 * S-Board boot manager but with MCANA on GPIO 0 (RX) / GPIO 1 (TX) per
 * the BU board hardware (matches src/can_bu.c in the BU application).
 * If the BU's status LED is not on GPIO 21, adjust ledInit/ledSet.
 *
 * Lives in Bank 0 Sectors 0-7 (0x080000-0x081FFF).
...
#define CAN_ID_FLAG_STATUS      0x0AU   /* flag values read from Bank 3 */
#define CAN_ID_FAPI_INIT_DONE   0x0BU   /* Fapi_initializeAPI returned */
#define CAN_ID_BANK_SET_DONE    0x0CU   /* Fapi_setActiveFlashBank returned */
#define CAN_ID_CRC_DONE         0x0DU   /* computeCRC32 finished, value attached */
#define CAN_ID_CRC_RESULT       0x19U   /* CRC compare done (match/mismatch) */
#define CAN_ID_ERASE_PROG       0x1AU   /* erase loop about to start */
#define CAN_ID_COPY_DONE        0x1BU   /* copy + clear-flag completed */
#define CAN_ID_NO_UPDATE        0x1CU   /* no flag set, jumping to app */
#define CAN_ID_CRC_FAIL         0x1DU   /* CRC mismatch, skipping copy */
/* New granular trace points — every step that can fail silently or hang */
#define CAN_ID_ERASE_PROGRESS   0x20U   /* every N sectors during erase loop */
#define CAN_ID_ERASE_FAIL       0x21U   /* Fapi error on a sector erase */
#define CAN_ID_PROG_PROGRESS    0x22U   /* every N×8-word writes during program */
#define CAN_ID_PROG_FAIL        0x23U   /* Fapi error on a programming command */
#define CAN_ID_COPY_FINISHED    0x24U   /* program loop finished */
#define CAN_ID_CLEAR_FLAG_BEGIN 0x25U   /* about to erase Bank 3 flag sector */
#define CAN_ID_CLEAR_FLAG_DONE  0x26U   /* flag erase finished */
#define CAN_ID_RESETTING        0x27U   /* about to call SysCtl_resetDevice */
#define CAN_ID_JUMPING_TO_APP   0x28U   /* about to call ((void(*)())0x082000)() */

/* How often to broadcast progress during long loops. */
#define ERASE_PROGRESS_EVERY    16U     /* every 16 sectors (out of 120) */
#define PROG_PROGRESS_EVERY     128U    /* every 128 × 8-word writes = 2 KB */
 ...
 * eraseSector / programEightWords now return the FAPI status code so
 * the caller can broadcast a debug frame on failure instead of
 * silently continuing with a half-written copy. */
static void              clearFSM(void);
static Fapi_StatusType   eraseSector(uint32_t sectorAddr);
...
void main(void)
{
    ...
    {
        Fapi_StatusType copy_st, clear_st;
        ...
        EDIS;
        canSendDebug(CAN_ID_FAPI_INIT_DONE, 0x01U, 0U,
                     DEVICE_SYSCLK_FREQ / 1000000U, 0U);

        EALLOW;
        Fapi_setActiveFlashBank(Fapi_FlashBank0);
        EDIS;
        canSendDebug(CAN_ID_BANK_SET_DONE, 0x01U, 0U, 0U, 0U);

        /* Verify Bank 2 CRC */
        computedCRC = computeCRC32(BANK2_START, imageSize);
        canSendDebug(CAN_ID_CRC_DONE, 0x01U, 0U, imageSize, computedCRC);
        ...
        if (computedCRC == imageCRC)
        {
             ...
             * copyBank2ToBank0 broadcasts its own progress / fail
             * frames (CAN IDs 0x20 / 0x21 / 0x22 / 0x23 / 0x24) so
             * we know exactly which step inside the loop failed. */
            EALLOW;
            copy_st = copyBank2ToBank0(imageSize);
            EDIS;

            canSendDebug(CAN_ID_COPY_DONE,
                         (copy_st == Fapi_Status_Success) ? 0x01U : 0xFFU,
                         (uint8_t)copy_st,
                         imageSize, (uint32_t)copy_st);

            if (copy_st == Fapi_Status_Success)
            {
                canSendDebug(CAN_ID_CLEAR_FLAG_BEGIN, 0x01U, 0U,
                             BANK3_FLAG_ADDR, 0U);
                ...
                canSendDebug(CAN_ID_CLEAR_FLAG_DONE,
                             (clear_st == Fapi_Status_Success) ? 0x01U : 0xFFU,
                             (uint8_t)clear_st,
                             BANK3_FLAG_ADDR, (uint32_t)clear_st);

                canSendDebug(CAN_ID_COPY_DONE, 0x02, 0x00, 0, 0);

                ledSet(0U);
                canSendDebug(CAN_ID_RESETTING, 0x01U, 0U, 0U, 0U);
                /* tiny delay so the frame physically leaves the bus
                 * before we yank the chip into reset */
            ...
            {
                /* Copy failed — leave the flag set so the boot manager
                 * tries again on next reset (better than wiping the
                 * flag and ending up with half-written Bank 0). */
                ledSet(0U);
            }
        }
        else
        {
            ...
            canSendDebug(CAN_ID_CLEAR_FLAG_BEGIN, 0x02U, 0U,
                         BANK3_FLAG_ADDR, 0U);
            EALLOW;
            clear_st = clearBootFlag();
            EDIS;
    ...
    canSendDebug(CAN_ID_JUMPING_TO_APP, 0x01U, 0U, APP_ENTRY_ADDR, 0U);
    jumpToApp();
}
 ...
 *
 *  This matches the LaunchXL-F28P55x Launchpad dev board AND the
 *  S-Board production hardware, so a boot manager built from this
 *  file works on both. Real BU-Board production hardware (64-pin
 *  F28P550SJ9) routes MCANA to GPIO 1 (TX) / GPIO 0 (RX) — if you
 *  port there, swap the two GPIO_setPinConfig calls below.
 *
 *  NOTE: boot_manager is its own standalone CCS project and does
 *  NOT include fw_upgrade_config.h (tight 8 KB flash budget, no
 *  extra include paths). Pins are hardcoded here on purpose.
 * ══════════════════════════════════════════════════════════════ */
static void configureMCANA(void)
{
...
static Fapi_StatusType eraseSector(uint32_t sectorAddr)
{
    ...
    fsm = Fapi_getFsmStatus();
    if (fsm != 3U)
        return Fapi_Error_AsyncIncorrectDataBufferLength;  /* sentinel non-zero */
...
static Fapi_StatusType programEightWords(uint32_t destAddr, const uint16_t *src)
{
    ...
    st = Fapi_issueProgrammingCommand((uint32_t *)destAddr,
                                      (uint16_t *)src, 8U,
...
{
    Fapi_StatusType st;
    uint32_t sectorAddr, srcAddr, dstAddr, wordsRemaining, wordsTotal;
    uint16_t i;
    uint32_t writeCount = 0U;

    /* ── Erase application area (sectors 8..127) ─────────────── */
    for (i = 0U; i < APP_SECTOR_COUNT; i++)
    {
        sectorAddr = BANK0_APP_START + ((uint32_t)i * SECTOR_SIZE_WORDS);

        st = eraseSector(sectorAddr);
        if (st != Fapi_Status_Success)
        {
            /* Erase failed — broadcast which sector and what code. */
            canSendDebug(CAN_ID_ERASE_FAIL,
                         (uint8_t)i, (uint8_t)st,
                         sectorAddr, (uint32_t)st);
            return st;
        }

        if (((uint32_t)i % ERASE_PROGRESS_EVERY) == 0U)
        {
            canSendDebug(CAN_ID_ERASE_PROGRESS,
                         (uint8_t)i, (uint8_t)APP_SECTOR_COUNT,
                         sectorAddr, (uint32_t)i);
        }
    }
    /* Final erase progress so the host sees we exited the loop. */
    canSendDebug(CAN_ID_ERASE_PROGRESS,
                 (uint8_t)APP_SECTOR_COUNT, (uint8_t)APP_SECTOR_COUNT,
                 0U, 0U);
    ...
    wordsTotal     = wordsRemaining;

    while (wordsRemaining >= 8U)
    {
        st = programEightWords(dstAddr, (const uint16_t *)srcAddr);
        if (st != Fapi_Status_Success)
        {
            canSendDebug(CAN_ID_PROG_FAIL,
                         (uint8_t)st, (uint8_t)(writeCount & 0xFFU),
                         dstAddr, (uint32_t)st);
            return st;
        }
        ...
        if ((writeCount % PROG_PROGRESS_EVERY) == 0U)
        {
            canSendDebug(CAN_ID_PROG_PROGRESS,
                         (uint8_t)(writeCount & 0xFFU),
                         (uint8_t)((writeCount >> 8) & 0xFFU),
                         dstAddr, wordsTotal - wordsRemaining);
        }
    }
    ...
    if (wordsRemaining > 0U)
    {
        ...
        st = programEightWords(dstAddr, padBuf);
        if (st != Fapi_Status_Success)
        {
            canSendDebug(CAN_ID_PROG_FAIL,
                         (uint8_t)st, 0xFFU,
    ...
    /* Loop completed cleanly — broadcast bytes copied. */
    canSendDebug(CAN_ID_COPY_FINISHED, 0x01U, 0U,
                 wordsTotal * 2U, writeCount);
    return Fapi_Status_Success;
}
