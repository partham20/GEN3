...
//    float tR = APPLY_GAIN(gMetrologyWorkingData.phases[3].readings.RMSVoltage, cal->primaryVoltage.R);
//    float tY = APPLY_GAIN(gMetrologyWorkingData.phases[4].readings.RMSVoltage, cal->primaryVoltage.Y);
//    float tB = APPLY_GAIN(gMetrologyWorkingData.phases[5].readings.RMSVoltage, cal->primaryVoltage.B);
//
//    /* L-L = sqrt(3) * L-N */
//    *vRY = 1.7320508f * tR;
//    *vYB = 1.7320508f * tY;
//    *vBR = 1.7320508f * tB;
    ...
    *vRY = (rawRY == 0.0f) ? sqrtf(tR * tR + tY * tY + tR * tY)
                            : APPLY_GAIN(rawRY, cal->primaryVoltage.R);
    *vYB = (rawYB == 0.0f) ? sqrtf(tY * tY + tB * tB + tY * tB)
                            : APPLY_GAIN(rawYB, cal->primaryVoltage.Y);
    *vBR = (rawBR == 0.0f) ? sqrtf(tB * tB + tR * tR + tB * tR)
                            : APPLY_GAIN(rawBR, cal->primaryVoltage.B);
...
//    PDUData *cal = &pduManager.workingData.pduData;
//
//    float tR = APPLY_GAIN(gMetrologyWorkingData.phases[0].readings.RMSVoltage, cal->secondaryVoltage.R);
//    float tY = APPLY_GAIN(gMetrologyWorkingData.phases[1].readings.RMSVoltage, cal->secondaryVoltage.Y);
//    float tB = APPLY_GAIN(gMetrologyWorkingData.phases[2].readings.RMSVoltage, cal->secondaryVoltage.B);
//
//    /* L-L = sqrt(3) * L-N */
//    *vRY = 1.7320508f * tR;
//    *vYB = 1.7320508f * tY;
