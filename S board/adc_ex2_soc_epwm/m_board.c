...
void sendMissingFrames(const uint8_t *retransmissionData)
{
    ...
    for (i = 0; i < 15; i++)
    {
        ...
        {  // This frame needs retransmission
            ...
            canTxWaitBufComplete(CAN_MBOARD_BASE, (1U << i));
            ...
            MCAN_writeMsgRam(CAN_MBOARD_BASE, MCAN_MEM_TYPE_BUF, i, &txMsg);

            // Request transmission
            MCAN_txBufAddReq(CAN_MBOARD_BASE, i);
 ...
 * Wire format: PDUData (451 words) in 15 CAN-FD frames.
...
void sendCalibrationDataToMBoard(void)
{
    ...
    if (newestData > 0)
    {
        ...
        pduManager.readWriteData.pduData.primaryVoltage = flashData->pduData.primaryVoltage;
        pduManager.readWriteData.pduData.secondaryVoltage = flashData->pduData.secondaryVoltage;
        pduManager.readWriteData.pduData.primaryCurrent = flashData->pduData.primaryCurrent;
        pduManager.readWriteData.pduData.secondaryCurrent = flashData->pduData.secondaryCurrent;
        pduManager.readWriteData.pduData.primaryKW = flashData->pduData.primaryKW;
        pduManager.readWriteData.pduData.secondaryKW = flashData->pduData.secondaryKW;
    ...
    // Serialize PDUData into CAN frames
    prepareCalibrationFramesForTx(&pduManager.readWriteData.pduData);

    // Transmit all 15 frames to M-board on MCANB (CAN ID 2)
    int i;
