...
/**
 * @brief Wait for CAN TX to complete with exact 3-second hardware timeout.
 *        If timeout expires, sends reset command to BU boards and resets the MCU.
 * @param base MCAN module base address (MCANA_DRIVER_BASE or MCANB_DRIVER_BASE)
 */
...
/**
 * @brief Wait for specific CAN TX buffer(s) to complete with exact 3-second timeout.
...
void canTxTimeoutReset(void)
{
    ...
    MCAN_writeMsgRam(MCANA_DRIVER_BASE, MCAN_MEM_TYPE_BUF, 0, &resetMsg);
    ...
    for (d = 0; d < 1500000; d++)
    {
        if (!MCAN_getTxBufReqPend(MCANA_DRIVER_BASE)) break;
    }
...
void MCANAConfig(void)
{
    ...
    // Buffer 3 is skipped (used by MCANB for M-Board ID 3 in shared rxMsg array)
    int i;
    for (i = 0; i < MAX_BU_BOARDS; i++)
    {
        stdFiltelem.sfid1 = FIRST_BU_ID + i;   // CAN ID: 11, 12, ..., 22
        stdFiltelem.sfid2 = 4 + i;             // Buffer:  4,  5, ..., 15
        MCAN_addStdMsgIDFilter(MCANA_DRIVER_BASE, 1 + i, &stdFiltelem);
    }
...
void MCANBConfig(void)
{
    ...
    // Wait for memory initialization to happen
    while (FALSE == MCAN_isMemInitDone(MCANB_DRIVER_BASE))
    {
    }
    ...
    // Wait till MCAN is initialized
    while (MCAN_OPERATION_MODE_SW_INIT != MCAN_getOpMode(MCANB_DRIVER_BASE))
    {
    }
    ...
    // Configure Bit timings
    MCAN_setBitTime(MCANB_DRIVER_BASE, &bitTimes);

    // Configure Message RAM Sections
    MCAN_msgRAMConfig(MCANB_DRIVER_BASE, &msgRAMConfigParams);
    ...
    stdFiltelem.sfid2 = 3;
    stdFiltelem.sfid1 = 3;
    MCAN_addStdMsgIDFilter(MCANB_DRIVER_BASE, 0, &stdFiltelem);
    ...
    stdFiltelem.sft = MCAN_STDFILT_RANGE;
    MCAN_addStdMsgIDFilter(MCANB_DRIVER_BASE, 20U, &stdFiltelem);
    ...
    MCAN_addStdMsgIDFilter(MCANB_DRIVER_BASE, 21U, &stdFiltelem);

    MCAN_setOpMode(MCANB_DRIVER_BASE, MCAN_OPERATION_MODE_NORMAL);

    while (MCAN_OPERATION_MODE_NORMAL != MCAN_getOpMode(MCANB_DRIVER_BASE))
    {
    }
}
...
void MCANIntrConfig(void)
{
    ...
    Interrupt_register(INT_MCANB_1, &MCANIntr1ISR);
...
void resetCANModule(void)
{
    ...
    MCAN_writeMsgRam(MCANB_DRIVER_BASE, MCAN_MEM_TYPE_BUF, 0, &TxMsgA);
    MCAN_txBufAddReq(MCANB_DRIVER_BASE, 0U);
...
void flushCalibrationFIFO(void)
{
    ...
    MCAN_getRxFIFOStatus(MCANB_DRIVER_BASE, &fifoStatus);

    while (fifoStatus.fillLvl > 0)
    {
        MCAN_writeRxFIFOAck(MCANB_DRIVER_BASE, MCAN_RX_FIFO_NUM_0, fifoStatus.getIdx);
        MCAN_getRxFIFOStatus(MCANB_DRIVER_BASE, &fifoStatus);
    }
...
/**
 * @brief Copy data from RX FIFO to buffer and process
...
int copyRxFIFO(void)
{
    ...
    #define HEADER_SIZE 2  // First two bytes are order information
    ...
    for (i = 0; i < MAX_MESSAGES; i++)
    {
        ...
        // Copy data from MCANB calibration buffer to receivedBuffer
    ...
    if (RxBufferToStruct())
    {
        // Wire CRC valid — write parsed struct to flash
        Example_ProgramUsingAutoECC();

        if (!updateFailed)
        {
            // Sync data after successful flash write
            syncReadWriteToWorking();

            // Copy data to BU boards and prepare for transmission
            copyFlashToBUBoards();

            // Send data to BU boards and wait for all ACKs
            bool allBUAcked = sendAllBoardsCalibrationDataWithRetry();

            // Send success/fail ACK to M-Board based on BU board responses
            if (allBUAcked)
            {
    ...
    {
        // Wire CRC mismatch — data from M-Board is corrupt
        calibDataUpdateFailAck();
    }
        ...
        void transmit(void)
        {
            ...
            for (i = 1; i < 16; i++)
            {
                if (retransmissionData[i] == 0x00)
                {
                    ...
                    canTxWaitBufComplete(MCANA_DRIVER_BASE, (1U << frameIndex));
                    ...
                    MCAN_writeMsgRam(MCANA_DRIVER_BASE, MCAN_MEM_TYPE_BUF, frameIndex, &txMsg);
                    MCAN_txBufAddReq(MCANA_DRIVER_BASE, frameIndex);
        ...
        void syncCANSignal(void)
               {
                   ...
                   canTxWaitComplete(MCANA_DRIVER_BASE);
               }
