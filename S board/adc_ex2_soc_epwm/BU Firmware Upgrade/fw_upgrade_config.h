 ...
 * both the S-Board and BU-Board projects — if you edit one, edit the
 ...
 *   - BU board ID range
 *   - M-Board → S-Board command opcodes that trigger the pipeline
 *
...
/* GPIO pin config — configured via can_config.h (CAN_BU_TX_PIN /
...
/* FD mode and bit-rate switching — both boards must agree. */
#define FW_CAN_FD_ENABLED           1U
#define FW_CAN_BRS_ENABLED          1U

/* Nominal (arbitration) phase — targets 500 kbps with 30 MHz clock:
...
/* ═══════════════════════════════════════════════════════════════
 *  CAN IDs  (S-Board ↔ BU OTA — disjoint from existing traffic)
 *
 ...
 *    0x18..0x1A   M-Board → S-Board BU image staging
 * ═══════════════════════════════════════════════════════════════ */

/* ── Hop 1: Host (PC / M-Board) → S-Board  ────────────────────────
 * Used by fw_bu_image_rx on the S-Board to stage a BU .bin into
 * Bank 1 before fw_bu_master re-transmits it to a BU board. */
#define FW_BU_STAGE_DATA_ID         0x18U   /* host → S-Board: data frames  */
#define FW_BU_STAGE_CMD_ID          0x19U   /* host → S-Board: commands     */
#define FW_BU_STAGE_RESP_ID         0x1AU   /* S-Board → host: responses    */

/* ── Hop 2: S-Board → BU-Board  ───────────────────────────────────
 * Used by fw_bu_master (sender, on S-Board) and fw_update_bu
 * (receiver, on BU) to stream Bank 1 contents into BU Bank 2. */
#define FW_BU_DATA_ID               0x30U   /* S-Board → BU:  data frames   */
#define FW_BU_CMD_ID                0x31U   /* S-Board → BU:  commands      */
#define FW_BU_RESP_ID               0x32U   /* BU → S-Board:  responses     */

/* Broadcast target — every BU sees it; addressing is done by
 * comparing data[1] against the BU's own board id. */
#define FW_BU_BROADCAST_TARGET      0xFFU

/* ── S-Board → M-Board: BU FW status reply channel ──────────────
 ...
 * ID 3 command channel (M-Board → S-Board) so replies do not loop
...
/* ── Hop 1: Host → S-Board (staging into Bank 1) ────────────────── */
...
/* ── Hop 2: S-Board → BU-Board (streaming into BU Bank 2) ───────── */
...
/* Burst size — number of data frames streamed before the sender
 ...
 *
 * Data frames carry RAW firmware bytes — bytes [0..63] are firmware,
 ...
 * contiguous 4×8-word (64-byte) flash chunk per frame with no
...
/* Ring buffer depth (ISR → main loop hand-off). Must be a power of 2.
...
/* S-Board side — Bank 1 is the staging area for a BU image delivered
 * by the host GUI. Never copied into S-Board Bank 0 — only re-
 * transmitted out to BU boards by fw_bu_master. */
#define FW_S_BANK1_START            0x0A0000UL
#define FW_S_BANK1_SECTORS          128U

/* BU-Board side — Bank 2 is the staging area for the new application
 * image pushed by the S-Board. The BU boot manager copies Bank 2 →
 * Bank 0 sectors 8..127 once the boot flag is set. */
#define FW_BU_BANK2_START           0x0C0000UL
#define FW_BU_BANK2_SECTORS         128U

/* BU-Board side — Bank 3 sector 0 holds the 8-word boot flag that
 * the boot manager checks on every reset. */
#define FW_BU_BANK3_FLAG_ADDR       0x0E0000UL

/* BU-Board application entry point — must match boot_manager.c
 ...
 * filter in fw_update_bu.c — the S-Board sends frames with target
 ...
 *
 * Does NOT affect the S-Board / fw_bu_master side — that code only
 * reads IDs from CAN command payloads, it never cares what the
 * local board id is. */
#define FW_BU_BOARD_ID_OVERRIDE     11U

/* ═══════════════════════════════════════════════════════════════
 *  M-BOARD → S-BOARD COMMAND OPCODES
 *
