...
#pragma DATA_SECTION(voltage, ".bss:large_arrays")
extern float voltage[SAMPLES_PER_CYCLE];
/*! @brief Array to store current debug data    */
#pragma DATA_SECTION(current, ".bss:large_arrays")
...
extern int bufferCount10;

extern signed int raw_value_ADC[6];   // Buffer for results
extern uint16_t index,set;                              // Index into result buffer
extern volatile uint16_t bufferfull;                // Flag to indicate buffer is full
extern signed int ADCResults[RESULTS_BUFFER_SIZE],ADCResults_Ground;
extern signed long adcAResults_mul[16];
extern unsigned long dwSumPhaseVolt[16],dwMathWordBuff,dwMathWordBuff1;
extern unsigned int voltage_gain,gain,wMathWordBuff;
extern unsigned int FanVolt[6],vgain,ADC_SIGNAL,b;
extern unsigned int count;

extern uint16_t windowCounter ; 
extern unsigned int counter200Ms;
