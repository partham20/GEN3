...
// =============================================================================
#define CAN_MODULE_BASE         CAN_BU_BASE         // MCAN module base address
...
void sendAllPhaseVoltages(void)
{
    ...
    /*
     * Get calibrated Vrms from metrology for each phase.
     * Apply calibration gain before sending to BU boards.
     */
    {
        PDUData *cal = &pduManager.workingData.pduData;
        vrmsR = (uint16_t)(gMetrologyWorkingData.phases[0].readings.RMSVoltage * cal->secondaryVoltage.R / 10000.0f);
        vrmsY = (uint16_t)(gMetrologyWorkingData.phases[1].readings.RMSVoltage * cal->secondaryVoltage.Y / 10000.0f);
        vrmsB = (uint16_t)(gMetrologyWorkingData.phases[2].readings.RMSVoltage * cal->secondaryVoltage.B / 10000.0f);
    }
...
void sendSingleVoltageFrame(uint16_t frameNumber, uint16_t *buffer, uint16_t startIndex, uint16_t phaseId, uint32_t canId, bool isLastFrame, uint16_t vrms)
{
    ...
    // Inter-frame delay: let bus idle between back-to-back frames to prevent
    // stuff errors when multiple BU boards are on the bus
    DEVICE_DELAY_US(100);
}
...
void initDAC(void)
{
    ...
    //
    // DAC output DISABLED — pin A0 is shared with ADCIN0 (output current R).
    // Enabling DACA drives pin A0, preventing the ADC from reading the
    // external current sensor.  Voltage playback to BU boards uses CAN,
    // not the DAC pin, so disabling this has no functional impact.
    //
}
 ...
 * Uncomment this and delete the simplified startup() above
 * when restoring full application functionality.
 */
#if 0
void startup_FULL(void)
{
        Device_init();
        Device_initGPIO();
        ...
        CPUTimer_setPeriod(CPUTIMER0_BASE, 150000000UL);
        CPUTimer_setPreScaler(CPUTIMER0_BASE, 0);
        CPUTimer_clearOverflowFlag(CPUTIMER0_BASE);
        CPUTimer_reloadTimerCounter(CPUTIMER0_BASE);
        CPUTimer_enableInterrupt(CPUTIMER0_BASE);
...
/* === ORIGINAL main() — disabled for FW update testing ===
 * Uncomment this and delete the simplified main() above
 * when restoring full application functionality.
 */
#if 0
void main_FULL(void)
{
    ...
    while (1)
    {
        ...
        if (buMessageReceived) { processBUMessages(); buMessageReceived = false; }

        if (bufferFull) { copyRxFIFO(); processReceivedCalibrationData(); }

        if (requestRetransmission) { handleMissingFrames(); requestRetransmission = false; stopSoftwareTimer(TIMER_FRAME_TRACKING); }

        if (!calibrationInProgress && runtimeActive)
        {
            if (runtimeMessageReceived) { processBURuntimeMessages(); runtimeMessageReceived = false; }
            if (activeBUMask != 0) {
                if (!buDataCollectionComplete && receivedCurrentFrames == activeBUMask && receivedKWFrames == activeBUMask) {
                    buDataCollectionComplete = true;
                    if (!buDataProcessed) { sendToMBoard = true; sendToBUBoard = false; syncBUBoardsData(); }
                }
            }
        }
        ...
        if (discoveryInProgress && timer10SecExpired)
        {
            ...
            if (activeBUMask != 0) {
                copyFlashToBUBoards();
                receivedCurrentFrames = 0; receivedKWFrames = 0;
                buDataCollectionComplete = false; buDataProcessed = false; sendToMBoard = false;
            } else {
                buDataCollectionComplete = true; buDataProcessed = true; sendToMBoard = true;
            }
            discoveryComplete = true;
        }
        ...
        if (discoveryComplete && !buDataCollectionComplete && activeBUMask != 0)
        {
            if (receivedCurrentFrames == activeBUMask && receivedKWFrames == activeBUMask)
            { buDataCollectionComplete = true; syncBUBoardsData(); buDataProcessed = true; sendToMBoard = true; }
        }
        ...
        if (CommandReceived)
        {
            ...
            {
                case CMD_START_DISCOVERY:
                    calibrationInProgress = true; runtimeActive = false;
                    resetCANModule(); ClearRxFIFOBuffer(); resetFrameTracking();
                    sendDiscoveryStartAck(); startNewDiscovery(); break;
                case CMD_READ:
                    findReadAndWriteAddress();
                    if (newestData > 0) { memcpy(&pduManager.readWriteData, (void*)readAddress, sizeof(CalibrationData)); }
                    prepareCalibrationFramesForTx(&pduManager.readWriteData.pduData);
                    handleRetransmissionRequest(&rxMsg[3]); break;
                case CMD_VERSION_CHECK: sendCalibrationDataFormatVersion(); break;
                case CMD_FLASH_ENABLE: calibrationInProgress = true; runtimeActive = false; enableFlashWrite(); break;
                case CMD_ERASE_BANK4:
                    calibrationInProgress = true; runtimeActive = false;
                    relayCommandToBUBoards(&rxMsg[3]); eraseBank4(); erasedBank4Ack(); break;
                case CMD_WRITE_DEFAULT:
                    relayCommandToBUBoards(&rxMsg[3]); findReadAndWriteAddress();
                    eraseSectorOrFindEmptySector(); writeDefaultCalibrationValues();
                    Example_ProgramUsingAutoECC(); writtenDefaultCalibrationValuesAck(); break;
                case CMD_STOP_CALIBRATION:
                    if (!updateFailed) { syncReadWriteToWorking(); if (activeBUMask != 0) { copyFlashToBUBoards(); } }
                    sendStopCalibrationToBUBoards(); sendStopCalibrationAck();
                    DEVICE_DELAY_US(10000); SysCtl_resetDevice(); break;
                case 0xFD: processSBoardRuntimeToMBoard(); break;
                default: break;
            }
            CommandReceived = false;
        }
        ...
        if (!calibrationInProgress && runtimeActive)
        {
            ...
            done = 0; dmaCompleteR = 0; dmaCompleteY = 0; dmaCompleteB = 0; isPlaybackMode = 0;
            DMA_clearTriggerFlag(DMA_CH_R_BASE); DMA_clearTriggerFlag(DMA_CH_Y_BASE); DMA_clearTriggerFlag(DMA_CH_B_BASE);
            ADC_clearInterruptStatus(ADC_MODULE_BASE, ADC_INT_PLAYBACK);
            ADC_clearInterruptStatus(ADC_MODULE_BASE, ADC_INT_R);
            ADC_clearInterruptStatus(ADC_MODULE_BASE, ADC_INT_Y);
            ADC_clearInterruptStatus(ADC_MODULE_BASE, ADC_INT_B);
            ...
            DMA_configAddresses(DMA_CH_Y_BASE, (uint16_t *)adcBufferY, (uint16_t *)ADC_RESULT_Y);
            DMA_configAddresses(DMA_CH_B_BASE, (uint16_t *)adcBufferB, (uint16_t *)ADC_RESULT_B);
            DMA_startChannel(DMA_CH_R_BASE); DMA_startChannel(DMA_CH_Y_BASE); DMA_startChannel(DMA_CH_B_BASE);
            EPWM_setTimeBaseCounter(EPWM_TRIGGER_BASE, 0);
            EPWM_enableADCTrigger(EPWM_TRIGGER_BASE, EPWM_SOC_TRIGGER);
            EPWM_setTimeBaseCounterMode(EPWM_TRIGGER_BASE, EPWM_COUNTER_MODE_UP);
            EPWM_disableADCTrigger(EPWM_TRIGGER_BASE, EPWM_SOC_TRIGGER);
            ...
            if(THD_ADC_isBufferReady())
            {
                int ch;
                for(ch = 0; ch < THD_NUM_CHANNELS; ch++)
                { THD_loadChannelData(&g_analyzer, ch, THD_ADC_getChannelBuffer(ch), THD_FFT_SIZE); }
                THD_processAllChannels(&g_analyzer);
                THD_ADC_resetAndRearm();
            }
            ...
            numDevices = I2C_Scanner_scanBus(TCA9555_ADDR_START, TCA9555_ADDR_END, TCA9555_REG_OUTPUT0);
            results = I2C_Scanner_getResults();
        }
